<template>
  <div class="char-layout">
    <div class="section-head">{{ store.data.alice.$好感阶段 }}</div>
    <div class="title-grid">
      <div v-for="(title, name) in store.data.alice.称号" :key="name" class="title-box">
        <span class="title-name">{{ name }}</span>
        <div class="title-effect">{{ title.效果 }}</div>
        <div class="title-quote">"{{ title.自我评价 }}"</div>
      </div>
      <div v-if="_.isEmpty(store.data.alice.称号)" class="title-box">
    ...
    <div class="attire-list">
      <div v-for="(description, slot) in store.data.alice.着装" :key="slot" class="attire-item">
