...
11. **创建项目目录**: 在角色卡工作区下创建 `src/` 子目录。提取角色卡后工作区路径为 `workspace/cards/<cardname>/`，在其下创建 `src/<模块名>/` 目录（如 `src/statusbar/`、`src/panel/` 等）。
    ...
    workspace/cards/<cardname>/
      ...
      world/
      src/                   ← 前端源码目录（新建）
        statusbar/           ← 示例：状态栏模块（有 index.html → HTML 模式 → iframe）
          App.vue
          index.ts
          index.html
        panel/               ← 示例：面板模块（无 index.html → JS 模式 → 主页面弹窗）
          App.vue
          index.ts
...
12. **复制对应模板**: 根据界面类型，从 `devkit/templates/` 复制文件到 `workspace/cards/<cardname>/src/<模块名>/`：
    - 面板 → 复制 `templates/panel/` 的 App.vue + index.ts + views/（**无 index.html**，JS 模式输出）
...
### index.html 约束（仅状态栏/消息楼层）

以下约束仅适用于有 `index.html` 的 HTML 模式模块（状态栏等）。面板无 `index.html`，不涉及此约束。
...
### iframe 适配规则（仅状态栏/消息楼层）

以下规则仅适用于以 iframe 形式嵌入酒馆消息楼层的前端界面（状态栏等）。面板通过 JS 模式在主页面运行，不受这些限制。
...
### iframe 全局变量桥接（仅状态栏/消息楼层，自动）

以下桥接机制仅适用于 HTML 模式构建（有 index.html 的模块，如状态栏）。面板使用 JS 模式（无 index.html），在酒馆主页面运行，全局变量天然可用，不需要桥接。
...
### `</script>` 和 `</style>` 转义（仅状态栏/消息楼层，自动）

以下转义机制仅适用于 HTML 模式构建。面板使用 JS 模式，不涉及此问题。
...
17. **构建**（强制）: 在 devkit 目录运行 `pnpm build`（或 `pnpm dev` 进入 watch 模式）。**禁止**跳过构建步骤直接手写编译产物。
    ...
    - 有 `index.html` 的模块 → 输出 `dist/<模块>/index.html`（完整 HTML，内联所有 JS/CSS + IframeBridgePlugin 桥接脚本，适用于状态栏等 iframe 场景）
    - 无 `index.html` 的模块 → 输出 `dist/<模块>/index.js`（ES module，适用于面板、流式楼层、纯脚本）
...
18. **集成到角色卡**: 根据界面类型选择集成方式。注意不同界面类型的构建产物不同：
    - **有 `index.html` 的模块**（状态栏）→ 构建产物是 `dist/<模块>/index.html`（完整 HTML 文件，已内联所有 JS/CSS/桥接脚本）
    - **无 `index.html` 的模块**（面板、流式楼层、纯脚本）→ 构建产物是 `dist/<模块>/index.js`（ES module JS 文件）

    **状态栏** → 添加一个正则脚本到角色卡：
    - 在工作区 `regex/` 目录添加一个正则，匹配 `<StatusPlaceHolderImpl/>`，替换为加载编译后 HTML 的代码
    - 确保开场白末尾包含 `<StatusPlaceHolderImpl/>` 占位符

    **面板** → 添加酒馆助手脚本 + 按钮，点击时弹出 Vue 面板：

    面板的构建产物是 **JS 文件**（`dist/<模块>/index.js`），不是 HTML。面板在酒馆主页面运行（非 iframe），全局变量天然可用，无需桥接。集成步骤：

    1. **先构建**：必须先运行 `pnpm build` 生成 `dist/<模块>/index.js`。**禁止**手动编译 Vue 代码
    2. **读取构建产物**：读取 `dist/<模块>/index.js` 的完整内容
    3. **创建包装脚本**：在 `scripts/` 目录创建 `.json` 元数据和 `-content.js` 包装脚本，将构建产物直接粘贴为 `-content.js` 的内容

    构建产物 `index.js` 本身已包含完整的面板逻辑（按钮事件绑定、callGenericPopup 弹窗、Vue app 挂载），直接作为 `-content.js` 使用即可。
    ...
    > **⚠️ 注意**: `callGenericPopup` 第一个参数必须用 `$(...)` 包装为 jQuery 对象，不能传裸 HTMLElement。按钮回调不会从 JSON 元数据自动绑定，必须在脚本中用 `eventOn(getButtonEvent('按钮名'), callback)` 手动绑定。
